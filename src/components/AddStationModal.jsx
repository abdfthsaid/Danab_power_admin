...
const AddStationModal = ({ isOpen, onClose, onAddStation, initialValues = null, onSubmit = null, mode = 'add' }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.totalSlots || isNaN(formData.totalSlots) || Number(formData.totalSlots) < 1) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ imei: '', name: '', iccid: '', location: '', totalSlots: '' })
      setSuccessMessage(data.message || 'Station added ✅')
...
const InputField = ({ label, name, value, onChange, error, placeholder, type = 'text', icon }) => (
  <div>
